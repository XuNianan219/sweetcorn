...
import { Calendar, MapPin, Save, Sparkles, Upload, X } from 'lucide-react';
...
export const CelebA: React.FC = () => {
  ...
  useEffect(() => {
    const saved = StorageService.getCelebSilhouette('a');
  ...
  const saveSilhouette = async () => {
    ...
    const ok = StorageService.setCelebSilhouette(currentUser.id, 'a', dataUrl);
    if (!ok) return;
  ...
  return (
    <div className="space-y-10 animate-fadeIn pb-16">
      <section className="gradient-ziyu min-h-[560px] rounded-[3.2rem] p-10 md:p-12 shadow-2xl relative overflow-hidden">
        <div className="absolute inset-0 bg-black/10" />
        {silhouetteUrl && (
          <img
            src={silhouetteUrl}
            className="absolute inset-0 w-full h-full object-contain z-[1] pointer-events-none drop-shadow-[0_25px_40px_rgba(0,0,0,0.3)]"
            alt="梓渝人物剪影"
          />
        )}
        ...
        <div className="relative z-10 h-full flex items-end">
          <div className="space-y-4 text-white">
            <div className="flex items-center gap-3 bg-white/20 backdrop-blur-md px-4 py-1.5 rounded-full w-fit border border-white/30">
              ...
              <span className="text-xs font-black tracking-widest uppercase">极速小鱼</span>
            </div>
            <h1 className="text-7xl md:text-8xl font-black drop-shadow-2xl">
              梓渝 <span className="text-3xl font-medium opacity-80">Ziyu</span>
            </h1>
      ...
      <section className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm">
        <h2 className="text-2xl font-black text-gray-900">梓渝展示页</h2>
        <p className="text-gray-500 mt-2">背景保持蓝色主题，支持叠加透明人物剪影。仅管理员可更换剪影，其他用户只能查看。</p>
      </section>
      ...
      {showUploader && (
        <div className="fixed inset-0 z-[200] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onPaste={handlePasteSilhouette}>
          <div className="w-full max-w-xl bg-white rounded-[2rem] p-6 space-y-4 relative">
            ...
            <h3 className="text-2xl font-black text-gray-900">梓渝剪影设置（管理员）</h3>
            <p className="text-sm text-gray-500">建议上传透明背景 PNG/WebP。支持文件选择和 Ctrl+V 粘贴。</p>
            <label className="w-full p-5 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 cursor-pointer flex items-center justify-center gap-2 text-gray-500 font-bold hover:border-blue-200">
              <Upload size={16} />
            ...
            <div className="rounded-2xl overflow-hidden border border-white/30 bg-gradient-to-r from-indigo-500 to-blue-400 h-72 relative flex items-center justify-center shadow-inner">
              {(preview || silhouetteUrl) && <img src={preview || silhouetteUrl} className="w-full h-full object-contain p-2" alt="preview" />}
              {!preview && !silhouetteUrl && <span className="text-white/80 text-sm font-bold">剪影预览区</span>}
            </div>
