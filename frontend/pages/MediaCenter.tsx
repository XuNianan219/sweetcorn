...
import { Bookmark, Heart, ImageIcon, MessageSquare, MoreHorizontal, Play, Repeat2, Search, Send, Upload, X } from 'lucide-react';
...
export const MediaCenter: React.FC = () => {
  ...
  const handleRepost = (item: MediaCard) => {
    const repostTags = Array.from(new Set([...(item.tags || []), '#转发']));
    const id = `${Date.now()}${Math.floor(Math.random() * 1000)}`;
    if (item.kind === 'photo') {
      StorageService.savePhoto({ id, creatorId: currentUser.id, title: `转发：${item.title}`, url: item.cover, author: currentUser.realName, likes: 0, tags: repostTags, commentsList: [] });
    } else {
      StorageService.saveVideo({ id, creatorId: currentUser.id, title: `转发：${item.title}`, author: currentUser.realName, likes: 0, comments: 0, cover: item.cover, tags: repostTags, commentsList: [], merchId: item.merchId });
    }
    load();
  };
