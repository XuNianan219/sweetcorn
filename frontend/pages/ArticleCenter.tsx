...
export const ArticleCenter: React.FC = () => {
  const { t } = useLang();
  const tc = (c: string) => t(c, CAT_EN[c] ?? c);
  ...
  const loadArticles = useCallback(async () => {
    ...
    try {
      ...
      setError(t('加载失败，请检查后端连接', 'Failed to load — check the server connection'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDelete = async (id: string, authorId: string) => {
    ...
    if (!window.confirm(t('文章将被删除，确认吗？', 'This article will be deleted. Continue?'))) return;
    try {
      await deletePost(id);
      await loadArticles();
    } catch {
      setError(t('删除失败，请重试', 'Delete failed, please retry'));
    }
  };
  ...
  const generateAIFic = async () => {
    if (!inspiration.trim()) {
      showInfo(t('请先在灵感区输入片段，AI 将为您扩写', 'Enter a snippet in the inspiration box and AI will expand it'));
      return;
    }
    ...
    try {
      ...
      if (!newTitle) setNewTitle(t('AI 创作灵感稿', 'AI draft'));
    } catch (err) {
      console.error(err);
      showError(t('扩写失败，请检查网络连接', 'Expansion failed — check your connection'));
    } finally {
  ...
  const publishArticle = async () => {
    if (!newTitle.trim() || !newContent.trim()) {
      showInfo(t('标题和内容是正式发布的前提', 'Title and content are required to publish'));
      return;
    }
    ...
    try {
      ...
      showSuccess(t('发布成功！', 'Published!'));
    } catch {
      setError(t('发布失败，请重试', 'Failed to publish, please retry'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-12 pb-20 animate-fadeIn">
      <div className="bg-white rounded-[3rem] p-10 shadow-sm border border-green-50 flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div>
          <h1 className="text-4xl font-black text-green-950 mb-4">{t('文学馆', 'Literature Hall')}</h1>
        ...
        >
          <PenTool size={24} /> {t('开启创作', 'Start writing')}
        </button>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder={t('搜索文学作品...', 'Search literature...')}
      ...
      {!loading && filteredArticles.length === 0 && (
        <div className="text-center py-12 text-gray-300 font-medium">{t('还没有文章，来发第一篇吧！', 'No articles yet — write the first one!')}</div>
      )}
            ...
            <div className="mt-auto pt-10 border-t border-gray-50 flex items-center justify-between">
              <div className="flex items-center gap-8">
                ...
                <button onClick={() => setActiveCommentId(activeCommentId === art.id ? null : art.id)} className="flex items-center gap-2 hover:text-blue-500 transition-colors text-gray-400">
                  ...
                  <span className="font-black text-lg">{t('评论', 'Comments')}</span>
                </button>
              </div>
              <span className="text-sm font-black text-gray-400 italic">
                BY @{art.author?.nickname ?? t('用户', 'User')}
              </span>
            </div>

            {activeCommentId === art.id && (
              <div className="mt-8 pt-8 border-t border-gray-50 space-y-4 animate-fadeIn">
                <p className="text-sm text-gray-400">{t('评论功能即将接入后端～', 'Comments coming soon~')}</p>
                <div className="flex gap-2 bg-gray-100 rounded-2xl p-2 pr-4">
                  <input type="text" value={commentText} onChange={(e) => setCommentText(e.target.value)} placeholder={t('写下你的读后感...', 'Share your thoughts...')} className="flex-grow bg-transparent p-3 outline-none text-sm font-medium" />
      ...
      {showForm && (
        <div className="fixed inset-0 z-[120] bg-black/70 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-[3.5rem] w-full max-w-6xl p-12 relative shadow-2xl overflow-y-auto max-h-[95vh] space-y-10">
            ...
            <h2 className="text-4xl font-black text-gray-950 text-center">{t('文学创作工坊', 'Writing Studio')}</h2>

            <div className="grid grid-cols-1 lg:grid-cols-5 gap-14">
              <div className="lg:col-span-2 space-y-10 border-r border-gray-100 pr-14">
                <div className="space-y-4">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest px-1">{t('创作类目', 'Category')}</label>
                  ...
                  {selectedCategory === '自定义' && (
                    <input type="text" value={customCategory} onChange={(e) => setCustomCategory(e.target.value)} placeholder={t('输入新类目名称...', 'Enter a new category name...')} className="w-full p-5 rounded-2xl bg-gray-50 border-none outline-none focus:ring-4 focus:ring-purple-100 font-medium" />
                  )}
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest px-1">{t('文章题目', 'Title')}</label>
                  <input type="text" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder={t('请输入心动的标题...', 'Enter an enticing title...')} className="w-full p-5 rounded-2xl bg-gray-50 border-none outline-none focus:ring-4 focus:ring-green-100 font-medium" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest px-1 flex items-center gap-2"><Hash size={14} /> {t('话题标签', 'Tags')}</label>
                  <input type="text" value={newTopic} onChange={(e) => setNewTopic(e.target.value)} placeholder={t('用空格或逗号分隔话题...', 'Separate tags with spaces or commas...')} className="w-full p-5 rounded-2xl bg-gray-50 border-none outline-none focus:ring-4 focus:ring-green-100 font-medium" />
                </div>
                <div className="p-8 bg-green-50 rounded-[2.5rem] border border-green-100 space-y-6 shadow-inner">
                  <h3 className="font-black text-green-900 flex items-center gap-3 text-sm"><Sparkles size={20} /> {t('创作灵感扩写区', 'AI inspiration expander')}</h3>
                  <textarea value={inspiration} onChange={(e) => setInspiration(e.target.value)} placeholder={t('提交灵感片段，AI将为您扩写...', 'Submit a snippet and AI will expand it...')} className="w-full h-40 p-5 rounded-2xl bg-white border-none outline-none focus:ring-4 focus:ring-green-300 font-medium text-sm resize-none shadow-sm" />
                  <button onClick={generateAIFic} disabled={isGenerating || !inspiration.trim()} className="w-full py-4 gradient-ningyuzhi text-green-950 font-black rounded-2xl text-sm disabled:opacity-50 hover:shadow-xl transition-all flex items-center justify-center gap-3">
                    {isGenerating ? <LayoutGrid className="animate-spin" size={20} /> : <Plus size={20} />} {t('提交灵感并扩写', 'Expand with AI')}
                  </button>
                </div>
              </div>
              ...
              <div className="lg:col-span-3 space-y-8">
                <div className="space-y-3">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest px-1">{t('正文编辑', 'Body')}</label>
                  <textarea value={newContent} onChange={(e) => setNewContent(e.target.value)} placeholder={t('扩写或创作的内容将在此显示，您可以随时润色修改...', 'Generated or written content appears here — edit freely...')} className="w-full h-[550px] p-10 rounded-[3rem] bg-gray-50 border-none outline-none focus:ring-4 focus:ring-green-400 font-medium leading-relaxed resize-none shadow-inner text-xl" />
                </div>
                <button onClick={publishArticle} disabled={submitting} className="w-full py-6 gradient-redsea text-white font-black text-2xl rounded-3xl shadow-2xl hover:scale-[1.02] transition-all disabled:opacity-50 flex items-center justify-center gap-3">
                  {submitting ? <Loader2 className="animate-spin" size={28} /> : t('确认正式提交发布', 'Publish')}
                </button>
              </div>
            </div>
          </div>
