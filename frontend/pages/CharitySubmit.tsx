    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center animate-fadeIn">
        <div className="text-center space-y-5 max-w-md">
          ...
          <p className="text-gray-500">{t('公益项目申请已进入审核队列，3 个工作日内反馈。', 'Your charity project is in the review queue — feedback within 3 business days.')}</p>
          <Link to="/activity/charity" className="inline-block px-8 py-3 rounded-2xl gradient-ningyuzhi font-black text-green-900">
            {t('返回公益区域', 'Back to charity')}
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 pb-20 animate-fadeIn">
      <div className="flex items-center justify-between">
        ...
        <h1 className="text-3xl font-black text-green-950">{t('公益项目提交通道', 'Charity Project Submission')}</h1>
        <div className="w-24" />
      </div>
      ...
      <div className="bg-white rounded-[3rem] p-10 md:p-14 shadow-2xl border border-green-50 space-y-8">
        <div className="flex items-start gap-3 p-5 bg-blue-50 rounded-2xl border border-blue-100 text-blue-700">
          ...
          <p className="text-sm font-bold">{t('请确保项目合法合规，具备可执行预算与时间节点。', 'Please ensure the project is legal and compliant, with a workable budget and timeline.')}</p>
        </div>
        ...
        >
          ...
          <input required className="w-full p-4 rounded-2xl bg-gray-50 outline-none" placeholder={t('公益项目名称', 'Charity project name')} />
          <textarea required className="w-full h-40 p-4 rounded-2xl bg-gray-50 outline-none resize-none" placeholder={t('项目简介、预算、执行计划...', 'Overview, budget, execution plan...')} />
